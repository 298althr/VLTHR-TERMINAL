...
export function ForexPage() {
  ...
  const [interval, setIntervalTime] = useState<string>('1day');
  ...
  useEffect(() => {
    const fetchRates = async () => {
      setLoading(true);
      try {
        const majors = catalog.map(p => p.split('/')[1]).filter(Boolean);
        const data = majors.length ? await forexAdapter.getLatestRates('USD', majors) : [];
        setRates(data);
      } catch (e) {
        console.error('Forex Rates Fetch Error:', e);
      } finally {
        setLoading(false);
      }
    };
    ...
    fetchRates();
    const timer = setInterval(fetchRates, 1800000); // 30 mins
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        const data = await forexAdapter.getHistory(base, quote, interval);
        setHistory(data);
      } catch (e) {
        console.error('Forex History Fetch Error:', e);
      } finally {
